...
class VisitSafetyTests(TestCase):
 ...
 def test_locked_item_cannot_be_excluded_from_scope(self):
  v=Visit.objects.create(institution=self.ins,inspector=self.user,date=date.today()); item=VisitNode.objects.create(visit=v,stable_id='i1',title='مقيد',node_type='ITEM',scope_locked=True); self.c.post('/visits/%s/items/%s/exclude/'%(v.pk,item.pk)); item.refresh_from_db(); self.assertFalse(item.excluded)
